import React from "react";
import Nav from "../Nav/Nav";
import "./Header.css";
import Typed from "typed.js";
import { useEffect, useRef } from "react";
import { FaAward, FaFileCode } from "react-icons/fa";

const Header = () => {
  const el = useRef(null);

  useEffect(() => {
    const typed = new Typed(el.current, {
      strings: ["18 years old", "a Web Developer", "a Front-end"],
      startDelay: 300,
      typeSpeed: 150,
      backDelay: 150,
      smartBackspace: true,
      showCursor: false,
      loop: true,
    });
  });

  return (
    <>
      <Nav />
      <div className="container-fluid " id="header-con">
        <div className="container d-flex justify-content-between">
          <div className="p__pad">
            <h6 className="h6__header">Hi! I'm</h6>
            <h3 className="fw-bolder">John Michael Buan</h3>
            <h5 className="pb-3">
              I am <span ref={el}></span>{" "}
            </h5>
            <span>
              Looking for an opportunity to harness my skills and an opportunity
              to learn to others
            </span>{" "}
            <br />
            <div className="m-3">
              <button className="btn btnBlue p-3 mx-3">Download CV</button>
              <button className="btn borderBlue p-3">Contact Me</button>
            </div>
          </div>

          <div className="p__pad1">
            <img
              className="michael1"
              src="https://imgs.search.brave.com/6Xe7z_rqxzBwJsOPJj0eLgH4VeqIvKd3yhU8Rb9PrXM/rs:fit:1200:1200:1/g:"
              alt="michael"
            />
          </div>
        </div>
        <div className="d-flex justify-content-center text-center">
          <div className="row py-5 w-75 bg-white shadow  Absolute circled">
            <div className="col d-flex" style={{ marginLeft: "4%" }}>
              <FaAward size="50px" className="mx-2 faIcon-color" />
              <div>
                <span style={{ fontWeight: "bold", fontSize: "17px" }}>8 year and 3 months</span> <br />
                <span className="subHeader">Freelance experience</span>
              </div>
            </div>
            <div className="col d-flex">
              <FaAward size="50px" className="mx-2 faIcon-color" />
              <div>
                <span style={{ fontWeight: "bold", fontSize: "17px" }}>8 year and 3 months</span> <br />
                <span className="subHeader">Freelance experience</span>
              </div>
            </div>
            <div className="col d-flex">
              <FaAward size="50px" className="mx-2 faIcon-color" />
              <div>
                <span style={{ fontWeight: "bold", fontSize: "17px" }}>8 year and 3 months</span> <br />
                <span className="subHeader">Freelance experience</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Header;
